(function (root) {
  ...
  function render(){
    ...
    var lista=D.filtrarAtas(state.atas,fs); var k=D.kpis(state.atas); var resps=Array.from(new Set(state.atas.map(function(a){return a.responsavel;}).filter(Boolean))).sort(); var anos=Array.from(new Set(state.atas.map(function(a){return String(a.anoAta||'');}).filter(Boolean))).sort().reverse(); var uasgs=Array.from(new Set(state.atas.map(function(a){return String(a.uasg||'');}).filter(Boolean))).sort();
